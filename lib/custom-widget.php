<?php
/**
 * Modified version of the Genesis Featured Posts Widget which extends the WP_Widget class.
 *
 * @author    Brad Dalton
 * @example   http://wpsites.net/best-plugins/custom-genesis-featured-posts-widget-you-can-modify-safely/
 */

class Custom_Genesis_Post extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname'   => 'genesis-content genesispost',
			'description' => __( 'Custom Genesis posts widget with thumbnails', 'genesis' ),
		);

		$control_ops = array(
			'id_base' => 'genesis-post',
			'width'   => 505,
			'height'  => 350,
		);

		parent::__construct( 'genesis-post', __( 'Genesis - Posts', 'genesis' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
            ...
            if (get_the_post_thumbnail(get_the_ID()) != '') :
                ...
                $image = '<img src="'.catch_that_image().'" class="card-img-top">';
            endif;
